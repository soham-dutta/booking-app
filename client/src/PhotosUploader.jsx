...
export default function PhotosUploader({ addedPhotos, onChange }) {
  ...
  async function addPhotoByLink(ev) {
    ev.preventDefault();
    try {
      // Assuming /upload-by-link endpoint is available on the backend
      const { data: filename } = await axios.post('http://localhost:4000/upload-by-link', { link: photoLink });
      onChange(prev => [...prev, filename]);  // Add new photo to the list
      setPhotoLink('');  // Clear input field
    } catch (error) {
      console.error("Failed to upload photo by link", error);  // Error handling
    }
  }
  ...
  function uploadPhoto(ev) {
    ...
    const data = new FormData();
    for (let i = 0; i < files.length; i++) {
      data.append('photos', files[i]);  // Append multiple files
    }
    axios.post('http://localhost:4000/upload', data, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
      .then(response => {
        const { data: filenames } = response;  // Get filenames from the response
        onChange(prev => [...prev, ...filenames]);  // Add new photos to the existing list
      })
      .catch(error => {
        console.error("Failed to upload photos", error);  // Error handling
      });
  }

  function removePhoto(ev, filename) {
    ev.preventDefault();
    onChange(addedPhotos.filter(photo => photo !== filename));  // Remove photo from the list
  }

  function selectAsMainPhoto(ev, filename) {
    ev.preventDefault();
    onChange([filename, ...addedPhotos.filter(photo => photo !== filename)]);  // Set the selected photo as the main one
  }
